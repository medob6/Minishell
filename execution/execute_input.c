...
static bool	init_cmd_structure(t_cmd *cmd, t_ast_node *node)
{
	if (!cmd || !node)
		return (false);
	cmd->is_built_in = 0;
...
static bool	set_cmd_args_and_path(t_cmd *cmd, char **cmd_args, t_env *env)
{
...
static void	reset_fds(t_data *data, int n)
{
	if (n != data->cmd_nbr - 1)
		close(data->fd[0]);
	if (data->old_fd != -1)
...
static bool	is_ambiguous_redirect(t_token *token)
{
...
static bool	open_output_redirect(t_data *data, t_token *token, int *last_idx, int idx)
{
...
static bool	open_input_redirect(t_data *data, t_token *token)
{
	data->out_fd = open_file(token);
	if (data->out_fd < 0)
	{
...
static bool	handle_redirect_token(t_data *data, t_token *token, int *last_idx, int idx)
{
	if (data->out_fd != 1)
		close(data->out_fd);
...
static bool	process_redirections(t_data *data, t_token **redir_lst,
		int *last_idx)
{
	size_t	i;
...
static bool	reopen_last_output(t_data *data, t_token **redir_lst, int last_idx)
{
	if (last_idx != -1)
	{
		close(data->out_fd);
...
void	child(t_data *prg_data, int index)
{
	...
	else
	{
		if (redirection_builtins(prg_data, index))
			prg_data->lst_cmd[index].exit_status = execute_built_in(prg_data->lst_cmd[index],
					prg_data);
	}
...
static void	init_program_data(t_data *data, t_ast_node *pipeline, t_env *env)
{
	...
	data->lst_cmd = parse_cmd_list(data->cmd_nbr,
